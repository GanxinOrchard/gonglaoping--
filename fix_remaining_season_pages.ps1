...
$footerContent = @"
    ...
    <!-- 手機版回到頂部按鈕 -->
    <button class="back-to-top mobile-only" id="backToTop" aria-label="回到頂部">
        <i class="fas fa-chevron-up"></i>
    </button>
"@

foreach ($page in $seasonPages) {
    if (Test-Path $page) {
        Write-Host "修復 $page..."
        ...
        # 替換舊的頁尾和回到頂部按鈕
        $content = $content -replace '<!-- 手機版回到頂部按鈕 -->.*?<script src="\./js/mobile-menu-simple\.js.*?</body>', $footerContent + "`n`n    <script src=`"./js/mobile-menu-simple.js?v=20250115`"></script>`n</body>"
